...
ラベルは `design` / `enhancement` / `bug` / `infra` / `content` / `engine` / `i18n` / `theme:tcp` / `theme:dns` / `theme:tls` / `phase:0`〜`phase:4`。
...
| アクターの種類 | `ActorKind` に `router` を足す | ARP・ICMP・NAT ではルーターが主役になり、クライアントともサーバーとも役割が違う。アクターの役割を型で正しく表すため |
| アドレスの割り当て | 5 テーマで共通（PC 192.168.1.10、ルーター 192.168.1.1 / 203.0.113.5、サーバー 192.0.2.10）。グローバルは文書用のアドレス（RFC 5737）、MAC は文書用の範囲（RFC 9542） | テーマをまたいで同じ機器だとわかる。実在の機器と衝突しない |
...
| リスク | 対策 |
|---|---|
| 技術的な内容の誤り（学習サイトでは致命的） | シナリオに RFC の参照コメントを必須にする（TCP: RFC 9293、DNS: RFC 1034/1035、TLS 1.3: RFC 8446、証明書: RFC 5280、ARP: RFC 826、DHCP: RFC 2131/2132、ICMP: RFC 792/1122/1812、NAT: RFC 3022/4787/5382、経路制御: RFC 1812/4632）。seq/ack やメッセージ順はテストで固定し、テーマごとにレビューの Issue を立てる |
